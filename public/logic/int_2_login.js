...
function loginPage() {
  ...
  loginForm.addEventListener("submit", async (event) => {
    ...
    try {
      const loginOk = await almacenaje.loguearUsuario(
      ...
      if (loginOk) {
        ...
        // Solo reseteamos si falla, para que el usuario no tenga que escribir todo de nuevo si se equivocó
        // loginForm.reset(); // Opcional, a veces es molesto que se borre todo
      }
    } catch (error) {
      console.error(error);
      loginMessage.innerHTML = `<div class="alert alert-danger mt-3">Error de conexión.</div>`;
    }
  });
}
